...
def users_view(request):
    if request.method == "POST":
        ...
        try:
            ...
            return JsonResponse(
                {"message": "Usuario actualizado correctamente"}, status=200
            )
    ...
    if request.method == "PUT":
        try:
            if request.user.has_perm("Reclutamiento.res_password"):
                return JsonResponse(
                    {"error": "No tienes permisos para restablecer la contraseña"},
                    status=403,
                )
            ...
            return JsonResponse(
                {"message": "Contraseña restablecida correctamente"}, status=200
            )
...
def dashboard_view(request):
    ...
    promedio_tiempo_efectivo_cobertura = plazas.filter(estatus="CERRADA").aggregate(
        Avg("tiempo_efectivo_cobertura")
    )["tiempo_efectivo_cobertura__avg"]
    ...
    plazas_con_edad = plazas.annotate(
        edad=current_year - ExtractYear("fecha_nacimiento")
    )

    promedio_edad = plazas_con_edad.aggregate(promedio=Avg("edad"))["promedio"]
    ...
    cesantias_count = (
        Cesantias.objects.values("estado_pago")
        .annotate(total=Count("id"))
        .filter(estado_pago__in=["AUTORIZADO", "CALCULADO", "PAGADO"])
    )
    estado_pago_counts = {
        estado["estado_pago"]: estado["total"] for estado in cesantias_count
    }

    # Nuevo conteo para BolsaEmpleos basado en el estado
    bolsa_estado_count = (
        BolsaEmpleos.objects.values("estado")
        .annotate(total=Count("id"))
        .filter(estado__in=["CONTACTADO", "DESCARTADO", "ENTREVISTADO", "REGISTRADO"])
    )
    bolsa_estado_counts = {
        estado["estado"]: estado["total"] for estado in bolsa_estado_count
    }
    ...
    context = {
        ...
        "estado_pago_labels": list(estado_pago_counts.keys()),
        "estado_pago_values": list(estado_pago_counts.values()),
        "bolsa_estado_labels": list(bolsa_estado_counts.keys()),
        "bolsa_estado_values": list(bolsa_estado_counts.values()),
    }
...
def empresas_view(request):
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_empresas"):
                return JsonResponse(
                    {"error": "No tienes permisos para crear sucursales"}, status=403
                )
            ...
            if not nombre_empresa:
                return JsonResponse(
                    {
                        "success": False,
                        "message": "El campo nombre_empresa es obligatorio.",
                    },
                    status=400,
                )
            ...
            if Empresas.objects.filter(nombre_empresa=nombre_empresa).exists():
                return JsonResponse(
                    {"success": False, "message": "La empresa ya está registrada."},
                    status=409,
                )

            empresa = Empresas.objects.create(
                nombre_empresa=nombre_empresa, activo=estado, usuario_creo=request.user
            )

            return JsonResponse(
                {"message": "Sucursal registrada correctamente"}, status=200
            )
        ...
        if not request.user.has_perm("Reclutamiento.change_empresas"):
            return JsonResponse(
                {"error": "No tienes permisos para actualizas"}, status=403
            )
        ...
        try:
            ...
            if (
                Empresas.objects.filter(nombre_empresa=nombre_empresa)
                .exclude(id=id)
                .exists()
            ):
            ...
            return JsonResponse(
                {"message": "Empresa actualizada correctamente"}, status=200
            )
    ...
    return JsonResponse(
        {"success": False, "message": "Metodo no permitido."}, status=405
    )
...
def sucursales_view(request):
    if request.method == "POST":
        try:
            ...
            if not nombre_sucursal:
                return JsonResponse(
                    {
                        "success": False,
                        "message": "El campo nombre sucursal es obligatorio.",
                    },
                    status=400,
                )
            ...
            if Sucursal.objects.filter(nombre_sucursal=nombre_sucursal).exists():
                return JsonResponse(
                    {"error": "La sucursal ya está registrada"}, status=409
                )
        ...
        if not request.user.has_perm("Reclutamiento.change_sucursal"):
            return JsonResponse(
                {"error": "No tienes permisos para actualizar sucursales"}, status=403
            )
        ...
        try:
            ...
            if (
                Sucursal.objects.filter(nombre_sucursal=nombre_sucursal)
                .exclude(id=sucursal_id)
                .exists()
            ):
                return JsonResponse(
                    {"error": "La sucursal ya está registrada"}, status=409
            ...
            return JsonResponse(
                {"message": "Sucursal actualizada correctamente"}, status=200
            )
...
def unidad_negocio_view(request):
    if request.method == "POST":
        if not request.user.has_perm("Reclutamiento.add_unidad_negocio"):
            return JsonResponse(
                {"error": "No tienes permisos para registrar sucursales"}, status=403
            )
        ...
        try:
            ...
            if not nombre_unidad_de_negocio:
                return JsonResponse(
                    {
                        "success": False,
                        "message": "El campo nombre unidad de negocio es obligatorio.",
                    },
                    status=400,
                )

            if Unidad_Negocio.objects.filter(
            ...
            return JsonResponse(
                {"success": True, "message": "Unidad de negocio creada exitosamente"},
                status=201,
            )
        ...
        try:
            ...
            if (
                Unidad_Negocio.objects.filter(
                    nombre_unidad_de_negocio=nombre_unidad_de_negocio
                )
                .exclude(id=unidad_id)
                .exists()
            ):
                return JsonResponse(
                    {"error": "La unidad de negocio ya esta registrada"}, status=409
                )
            ...
            return JsonResponse(
                {"message": "Unidad de negocio actualizada correctamente"}, status=200
            )

        except Unidad_Negocio.DoesNotExist:
            return JsonResponse(
                {"error": "Unidad de negocio no encontrada"}, status=404
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_departamento"):
                return JsonResponse(
                    {"error": "No tienes permisos para crear departamentos"}, status=403
                )
            ...
            if not nombre_departamento:
                return JsonResponse(
                    {
                        "success": False,
                        "message": "El campo nombre departamento es obligatorio.",
                    },
                    status=400,
                )
            ...
            if Departamento.objects.filter(
                ...
                return JsonResponse(
                    {"error": "El departamento ya está registrado"}, status=409
                )
            ...
            return JsonResponse(
                {"message": "Departamento registrado correctamente"}, status=200
            )
        ...
        if not request.user.has_perm("Reclutamiento.change_departamento"):
            return JsonResponse(
                {"error": "No tienes permisos para actualizar Departamentos"}
            )
        ...
        try:
            ...
            if (
                Departamento.objects.filter(nombre_departamento=nombre_departamento)
                .exclude(id=id)
                .exists()
            ):
                return JsonResponse(
            ...
            return JsonResponse(
                {"message": "Departamento actualizado correctamente"}, status=200
            )
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_ciudades"):
                return JsonResponse(
                    {"error": "No tienes permisos para registrar Ciudades"}
                )
            ...
            if not nombre_ciudades:
                return JsonResponse(
                    {
                        "success": False,
                        "message": "El campo nombre Ciudades es obligatorio.",
                    },
                    status=400,
                )
            ...
            if Ciudades.objects.filter(nombre_ciudades=nombre_ciudades).exists():
                return JsonResponse(
                    {"error": "La Ciudad ya está registrada"}, status=409
                )
            ...
            return JsonResponse(
                {"message": "Ciudad registrada correctamente"}, status=200
            )
        ...
        if not request.user.has_perm("Reclutamiento.change_ciudades"):
            return JsonResponse(
                {"error": "No tienes permisos para actualizar Ciudades"}
            )
        try:
            data = json.loads(request.body)
            ...
            if (
                Ciudades.objects.filter(nombre_ciudades=nombre_ciudades)
                .exclude(id=ciudades_id)
                .exists()
            ):
                return JsonResponse(
                    {"error": "La ciudad ya está registrada"}, status=409
            ...
            return JsonResponse(
                {"message": "Ciudad actualizada correctamente"}, status=200
            )
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_puestos"):
                return JsonResponse(
                    {"error": "No tienes permisos para registrar puestos"}
                )
            ...
            if Puestos.objects.filter(nombre_puestos=nombre_puestos).exists():
                return JsonResponse(
                    {"error": "El puesto ya está registrado"}, status=409
                )

            puesto = Puestos.objects.create(
                nombre_puestos=nombre_puestos, activo=activo, usuario_creo=request.user
            )

            return JsonResponse(
                {"message": "Puesto registrada correctamente"}, status=200
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
        ...
        try:
            if not request.user.has_perm("Reclutamiento.change_puestos"):
                return JsonResponse(
                    {"error": "No tienes permisos para actualizar puestos"}
                )
            ...
            if (
                Puestos.objects.filter(nombre_puestos=nombre_puestos)
                .exclude(id=puestos_id)
                .exists()
            ):
                return JsonResponse(
                    {"error": "El puesto ya está registrada"}, status=409
                )
            ...
            return JsonResponse(
                {"message": "Puesto actualizado correctamente"}, status=200
            )
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_prioridad"):
                return JsonResponse(
                    {"error": "No tienes permisos para registrar prioridades"}
                )
            ...
            if Prioridad.objects.filter(nombre_prioridad=nombre_prioridad).exists():
                return JsonResponse(
                    {"error": "La Prioridad ya está registrada"}, status=409
                )
            ...
            return JsonResponse(
                {"message": "Prioridad registrada correctamente"}, status=200
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
        ...
        try:
            if not request.user.has_perm("Reclutamiento.change_prioridad"):
                return JsonResponse(
                    {"error": "No tienes permisos para actualizar prioridades"}
                )
            ...
            if (
                Prioridad.objects.filter(nombre_prioridad=nombre_prioridad)
                .exclude(id=id_prioridad)
                .exists()
            ):
            ...
            return JsonResponse(
                {"message": "Prioridad actualizado correctamente"}, status=200
            )
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_modo"):
                return JsonResponse(
                    {"error": "No tienes permisos para registrar modos"}
                )
            ...
            modos = Modo.objects.create(
                nombre_modo=nombre_modo, activo=activo, usuario_creo=request.user
            )
            return JsonResponse(
                {"message": "Modo registrado correctamente"}, status=200
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
        ...
        try:
            if not request.user.has_perm("Reclutamiento.change_modo"):
                return JsonResponse(
                    {"error": "No tienes permisos para actualizar modos"}
                )
            ...
            if (
                Modo.objects.filter(nombre_modo=nombre_modo)
                .exclude(id=modo_id)
                .exists()
            ):
                return JsonResponse({"error": "El Modo ya está registrado"}, status=409)
            ...
            return JsonResponse(
                {"message": "Modo actualizado correctamente"}, status=200
            )
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_motivo"):
                return JsonResponse(
                    {"error": "No tienes permisos para registrar motivos"}
                )
            ...
            if Motivo.objects.filter(nombre_motivo=nombre_motivo).exists():
                return JsonResponse(
                    {"error": "El Motivo ya está registrado"}, status=409
                )

            motivos = Motivo.objects.create(
                nombre_motivo=nombre_motivo, activo=activo, usuario_creo=request.user
            )

            return JsonResponse(
                {"message": "Motivo registrada correctamente"}, status=200
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
        ...
        try:
            if not request.user.has_perm("Reclutamiento.change_motivo"):
                return JsonResponse(
                    {"error": "No tienes permisos para actualizar motivos"}
                )
            ...
            if (
                Motivo.objects.filter(nombre_motivo=nombre_motivo)
                .exclude(id=motivos_id)
                .exists()
            ):
                return JsonResponse(
                    {"error": "El motivo ya está registrada"}, status=409
                )
            ...
            return JsonResponse(
                {"message": "Motivo actualizado correctamente"}, status=200
            )
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_medioreclutamiento"):
                return JsonResponse(
                    {
                        "error": "No tienes permisos para registrar medios de reclutamiento"
                    }
                )
            ...
            if not nombre_medio_de_reclutamiento:
                return JsonResponse(
                    {"error": "El nombre del medio de reclutamiento es obligatorio"}
                )

            if MedioReclutamiento.objects.filter(
                nombre_medio_de_reclutamiento=nombre_medio_de_reclutamiento
            ).exists():
                return JsonResponse(
                    {"error": "El medio de reclutamiento ya está registrado"},
                    status=409,
                )
            ...
            return JsonResponse(
                {"message": "Medio de reclutamiento registrado correctamente"},
                status=200,
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
        ...
        try:
            if not request.user.has_perm("Reclutamiento.change_medioreclutamiento"):
                return JsonResponse(
                    {
                        "error": "No tienes permisos para actualizar medios de reclutamiento"
                    }
                )
            ...
            if (
                MedioReclutamiento.objects.filter(
                    nombre_medio_de_reclutamiento=nombre_medio_de_reclutamiento
                )
                .exclude(id=medio_reclutamiento_id)
                .exists()
            ):
                return JsonResponse(
                    {"error": "El medio de reclutamiento ya está registrado"},
            ...
            return JsonResponse(
                {"message": "Medio de reclutamiento actualizado correctamente"},
                status=200,
            )

        except MedioReclutamiento.DoesNotExist:
            return JsonResponse(
                {"error": "Medio de reclutamiento no encontrado"}, status=404
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
        ...
        if medio_reclutamiento_id:
            try:
                ...
                return JsonResponse(
                    {"error": "Medio de reclutamiento no encontrada"}, status=404
                )
        else:
            search = request.GET.get("search", "")
            estado = request.GET.get("estado", "")
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_tipocontrato"):
                return JsonResponse(
                    {"error": "No tienes permisos para registrar tipos de contrato"}
                )
            ...
            if not nombre_tipo_de_contrato:
                return JsonResponse(
                    {"error": "El nombre del tipo de contrato es obligatorio"}
                )

            if TipoContrato.objects.filter(
                nombre_tipo_de_contrato=nombre_tipo_de_contrato
            ).exists():
                return JsonResponse(
                    {"error": "El tipo de contrato ya está registrado"}, status=409
                )
            ...
            return JsonResponse(
                {"message": "Tipo de contrato registrado correctamente"}, status=200
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
        ...
        try:
            if not request.user.has_perm("Reclutamiento.change_tipocontrato"):
                return JsonResponse(
                    {"error": "No tienes permisos para actualizar tipos de contratos"}
                )
            ...
            if (
                ...
                .exclude(id=tipo_de_contrato_id)
                .exists()
            ):
                return JsonResponse(
                    {"error": "El tipo de contrato ya está registrado"}, status=409
            ...
            return JsonResponse(
                {"message": "Tipo de contrato actualizado correctamente"}, status=200
            )
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_spicosmart"):
                return JsonResponse(
                    {"error": "No tienes permisos para registrar pruebas psicosmart"}
                )
            ...
            if not nombre_prueba:
                return JsonResponse(
                    {"error": "El nombre de la prueba psicosmart es obligatorio"}
                )

            if Spicosmart.objects.filter(
                nombre_prueba=nombre_prueba, nivel=nivel
            ).exists():
                return JsonResponse(
                    {"error": "La prueba psicosmart ya está registrada"}, status=409
                )
            ...
            return JsonResponse(
                {"message": "Prueba psicosmart registrada correctamente"}, status=200
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
        ...
        try:
            if not request.user.has_perm("Reclutamiento.change_spicosmart"):
                return JsonResponse(
                    {"error": "No tienes permisos para actualizar pruebas psicosmart"}
                )
            ...
            if (
                Spicosmart.objects.filter(nombre_prueba=nombre_prueba)
                .exclude(id=prueba_id)
                .exists()
            ):
                return JsonResponse(
                    {"error": "La prueba psicosmart ya está registrada"}, status=409
            ...
            return JsonResponse(
                {"message": "Prueba psicosmart actualizada correctamente"}, status=200
            )

        except Spicosmart.DoesNotExist:
            return JsonResponse(
                {"error": "Prueba psicosmart no encontrada"}, status=404
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_pais"):
                return JsonResponse(
                    {"error": "No tienes permisos para registrar paises"}
                )
            ...
            pais = Pais.objects.create(
                nombre_pais=nombre_pais, activo=activo, usuario_creo=request.user
            )
            return JsonResponse(
                {"message": "Pais registrado correctamente"}, status=200
            )
        ...
        try:
            if not request.user.has_perm("Reclutamiento.change_pais"):
                return JsonResponse(
                    {"error": "No tienes permisos para actualizar paises"}
                )
            ...
            return JsonResponse(
                {"message": "Pais actualizado correctamente"}, status=200
            )
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_departamentopais"):
                return JsonResponse(
                    {"error": "No tienes permisos para registrar departamentos"}
                )
            ...
            if DepartamentoPais.objects.filter(
                nombre_departamento=nombre_departamento
            ).exists():
                return JsonResponse(
                    {"error": "El departamento ya está registrado"}, status=409
        ...
        try:
            ...
            if (
                DepartamentoPais.objects.filter(nombre_departamento=nombre_departamento)
                .exclude(id=departamentopais_id)
                .exists()
            ):
                return JsonResponse(
                    {"error": "El departamento ya está registrado"}, status=409
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_municipiopais"):
                return JsonResponse(
                    {"error": "No tienes permisos para registrar municipios"}
                )
            ...
            if MunicipioPais.objects.filter(nombre_municipio=nombre_municipio).exists():
                return JsonResponse(
                    {"error": "El municipio ya está registrado"}, status=409
                )
            ...
            return JsonResponse(
                {"message": "Municipio registrado correctamente"}, status=200
            )
        ...
        try:
            if not request.user.has_perm("Reclutamiento.change_municipiopais"):
                return JsonResponse(
                    {"error": "No tienes permisos para actualizar municipios"}
                )
            ...
            if (
                MunicipioPais.objects.filter(nombre_municipio=nombre_municipio)
                .exclude(id=municipio_id)
                .exists()
            ):
                return JsonResponse(
                    {"error": "El municipio ya está registrado"}, status=409
            ...
            return JsonResponse(
                {"message": "Municipio actualizado correctamente"}, status=200
            )
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_jefes"):
                return JsonResponse(
                    {"error": "No tienes permisos para registrar jefes"}
                )
            ...
            if Jefes.objects.filter(identidadjefe=identidadjefe).exists():
                return JsonResponse(
                    {"error": "La Identidad ingresada ya está registrada"}, status=409
                )
            ...
            jefe = Jefes.objects.create(
                ...
                empresa_id=empresa,
                unidad_de_negocio_id=unidadnegocio,
                departamento_id=departamento,
                puesto_id=puesto,
            )

            return JsonResponse(
                {"message": "Jefe registrado correctamente"}, status=200
            )
        ...
        try:
            if not request.user.has_perm("Reclutamiento.change_jefes"):
                return JsonResponse(
                    {"error": "No tienes permisos para actualizar jefes"}
                )
            ...
            jefe.sucursal_id = sucursal
            jefe.empresa_id = empresa
            jefe.unidad_de_negocio_id = unidadnegocio
            jefe.departamento_id = departamento
            jefe.puesto_id = puesto
            jefe.save()

            return JsonResponse(
                {"message": "Jefe actualizado correctamente"}, status=200
            )
        ...
        if jefe_id:
                ...
                return JsonResponse(
                    {
                        "jefe": {
                            ...
                            "activo": jefe.activo,
                            "sucursal": jefe.sucursal_id,
                            "empresa": jefe.empresa_id,
                            "unidadnegocio": jefe.unidad_de_negocio_id,
                            "departamento": jefe.departamento_id,
                            "puesto": jefe.puesto_id,
                        }
                    }
                )
            except Jefes.DoesNotExist:
            ...
            all_empresas = (
                Empresas.objects.filter(activo=True)
                .order_by("nombre_empresa")
                .distinct()
            )
            all_unidades_negocio = (
                Unidad_Negocio.objects.filter(activo=True)
                .order_by("nombre_unidad_de_negocio")
                .distinct()
            )
            all_departamentos = (
            ...
            all_puestos = (
                Puestos.objects.filter(activo=True)
                .order_by("nombre_puestos")
                .distinct()
            )
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_colaboradores"):
                return JsonResponse(
                    {"error": "No tienes permisos para registrar colaboradores"}
                )
            ...
            if Colaboradores.objects.filter(
                ...
                return JsonResponse(
                    {"error": "El colaborador ya está registrado"}, status=409
                )

            dni = data.get("dni")
            if not dni:
                return JsonResponse({"error": "El campo DNI es obligatorio"}, status=400)

            if Colaboradores.objects.filter(dni=dni).exists():
                return JsonResponse(
                    {"error": "La identidad ingresada ya existe"}, status=409
                )
            ...
            return JsonResponse(
                {"message": "Colaborador registrado correctamente"}, status=200
            )
        ...
        try:
            ...
            if (
                Colaboradores.objects.filter(
                    ...
                    sucursal_id=sucursal,
                    empresa_id=empresa,
                    unidad_de_negocio_id=unidad_de_negocio,
                    departamento_id=departamento,
                    jefe_id=jefes,
                    codigocolaborador=codigocolaborador,
                )
            ...
            colaborador.dni = dniedit
            colaborador.puesto_id = puesto
            ...
            return JsonResponse(
                {"message": "Colaborador actualizado correctamente"}, status=200
            )
        ...
        if colaborador_id:
            ...
            if search:
                colaboradores = colaboradores.filter(
                    ...
                    | models.Q(
                        unidad_de_negocio__nombre_unidad_de_negocio__icontains=search
                    )
                    | models.Q(departamento__nombre_departamento__icontains=search)
            ...
            all_jefes = (
                Jefes.objects.filter(activo=True).order_by("nombrejefe").distinct()
            )
            all_puestos = (
                Puestos.objects.filter(activo=True)
                .order_by("nombre_puestos")
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_controldetiempo"):
                return JsonResponse(
                    {"error": "No tienes permisos para registrar control de tiempo"}
                )
            ...
            unid_puesto = (
                f"{unidad_negocio.nombre_unidad_de_negocio}-{puesto.nombre_puestos}"
            )
            ...
            return JsonResponse(
                {
                    "success": True,
                    "message": "Control de Tiempo registrado correctamente.",
                }
            )
        ...
        try:
            if not request.user.has_perm("Reclutamiento.change_controldetiempo"):
                return JsonResponse(
                    {"error": "No tienes permisos para actualizar control de tiempo"}
                )
            ...
            if (
                ControlDeTiempo.objects.filter(
                    unidad_de_negocio_id=unidadnegocio,
                    departamento_id=departamento,
                    puestos_id=puestos,
                    tiempo=tiempo,
                )
                .exclude(id=controltiempo_id)
                .exists()
            ):
                return JsonResponse(
                    {"error": "Control de tiempo ya está registrada"}, status=409
                )
            ...
            return JsonResponse(
                {"message": "Control de tiempo actualizada correctamente"}, status=200
            )

        except ControlDeTiempo.DoesNotExist:
            return JsonResponse(
                {"error": "Control de tiempo no encontrada"}, status=404
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
        ...
        if control_tiempo_id:
            try:
                ...
                return JsonResponse(
                    {"error": "Control de Tiempo no encontrado"}, status=404
                )

        else:
            unidades_negocio = Unidad_Negocio.objects.filter(activo=True).order_by(
                "nombre_unidad_de_negocio"
            )
            departamentos = Departamento.objects.filter(activo=True).order_by(
                "nombre_departamento"
            )
            puestos = Puestos.objects.filter(activo=True).order_by("nombre_puestos")

            # Obtener todos los registros de ControlDeTiempo
            controles_tiempo = ControlDeTiempo.objects.all().prefetch_related(
                "unidad_de_negocio", "departamento", "puestos"
            )
...
def control_plazas_view(request, id=None):
    if request.method == "GET":
        ...
        if control_plaza_id:
            try:
                ...
                return JsonResponse(
                    {
                        "control_plaza": {
                            "id": control_plaza.id,
                            "usuario_creo": control_plaza.usuario_creo.get_full_name(),
                ...
                return JsonResponse(
                    {"error": "Control de Plazas no encontrado"}, status=404
                )
        else:
            ...
            mes_corte_options = set(
                control_de_plazas.values_list("mes_corte", flat=True)
            )
            mes_solicitud_options = set(
                control_de_plazas.values_list("mes_solicitud", flat=True)
            )
            ...
            motivos = (
                Motivo.objects.filter(activo=True).order_by("nombre_motivo").distinct()
            )
            prioridades = (
                Prioridad.objects.filter(activo=True)
                .order_by("nombre_prioridad")
                .distinct()
            )
            tipos_contrato = (
                TipoContrato.objects.filter(activo=True)
                .order_by("nombre_tipo_de_contrato")
                .distinct()
            )
            mediosReclutamiento = (
                MedioReclutamiento.objects.filter(activo=True)
                .order_by("nombre_medio_de_reclutamiento")
                .distinct()
            )
            ...
            unidades_negocio_options = (
                Unidad_Negocio.objects.filter(
                    id__in=control_de_plazas.values_list(
                        "unidad_de_negocio__id", flat=True
                    )
                )
                .distinct()
                .order_by("nombre_unidad_de_negocio")
            ...
            if sucursal_filter:
                control_de_plazas = control_de_plazas.filter(
                    sucursal__id__in=sucursal_filter
                )

            # Filtrar por Empresa
            if empresa_filter:
                control_de_plazas = control_de_plazas.filter(
                    empresa__id__in=empresa_filter
                )
            ...
            if mes_corte_filter:
                control_de_plazas = control_de_plazas.filter(
                    mes_corte__in=mes_corte_filter
                )

            # Filtrar por Mes de Solicitud
            if mes_solicitud_filter:
                control_de_plazas = control_de_plazas.filter(
                    mes_solicitud__in=mes_solicitud_filter
                )

            # Filtrar por Puesto
            if puesto_filter:
                control_de_plazas = control_de_plazas.filter(
                    puestos__id__in=puesto_filter
                )

            # Filtrar por Departamento
            if departamento_filter:
                control_de_plazas = control_de_plazas.filter(
                    departamento__id__in=departamento_filter
                )
            ...
            context = {
                ...
                "estatus_options": list(
                    estatus_options
                ),  # Convertir set a list para el contexto
                "año_options": list(
                    año_options
                ),  # Convertir set a list para el contexto
                "mes_corte_options": list(
                    mes_corte_options
                ),  # Convertir set a list para el contexto
                "mes_solicitud_options": list(
                    mes_solicitud_options
                ),  # Convertir set a list para el contexto
        ...
        if not request.user.has_perm("Reclutamiento.add_controldeplazas"):
            return JsonResponse(
                {"error": "No tienes permisos para crear plazas"}, status=403
            )
        try:
            ...
            unidad_negocio = Unidad_Negocio.objects.filter(
                id=data.get("unidad_de_negocio")
            ).first()
            ...
            motivo_ingreso = Motivo.objects.filter(
                id=data.get("motivo_ingreso")
            ).first()
            departamento = (
                Departamento.objects.filter(id=data.get("departamento")).first()
                if data.get("departamento")
                else None
            )
            prioridad = (
                Prioridad.objects.filter(id=data.get("prioridad")).first()
                if data.get("prioridad")
                else None
            )
            medio_reclutamiento = (
                MedioReclutamiento.objects.filter(
                    id=data.get("medio_reclutamiento")
                ).first()
                if data.get("medio_reclutamiento")
                else None
            )
            tipo_contrato = (
                TipoContrato.objects.filter(id=data.get("tipo_contrato")).first()
            ...
            fecha_solicitud = (
                parser.parse(data.get("fecha_solicitud"))
                if data.get("fecha_solicitud")
                else datetime.now()
            )
            fecha_cobertura = (
                parser.parse(data.get("fecha_cobertura"))
                if data.get("fecha_cobertura")
                else datetime.now()
            )
            fecha_ingreso = (
                parser.parse(data.get("fecha_ingreso"))
            ...
            tiempo_efectivo_cobertura = (
                (fecha_cobertura - fecha_solicitud).days
                if fecha_solicitud and fecha_cobertura
                else None
            )
            tiempo_efectivo_cobertura = (
                tiempo_efectivo_cobertura if tiempo_efectivo_cobertura > 0 else None
            )

            tiempo_efectivo_fecha_ingreso = (
                (fecha_ingreso - fecha_solicitud).days
                if fecha_solicitud and fecha_ingreso
                else None
            )
            tiempo_efectivo_fecha_ingreso = (
                tiempo_efectivo_fecha_ingreso
                if tiempo_efectivo_fecha_ingreso > 0
                else None
            )

            # Calcular fecha límite de cobertura
            fecha_limite_cobertura_str = (
                (fecha_solicitud + timedelta(days=tiempo_cobertura)).date()
                if fecha_solicitud and tiempo_cobertura
                else None
            )

            # Capturar y convertir las fechas
            fecha_limite_cobertura_cal = (
                datetime.combine(fecha_limite_cobertura_str, datetime.min.time())
                if fecha_limite_cobertura_str
                else None
            )
            fecha_cobertura_cal = (
                parser.parse(data.get("fecha_cobertura"))
            ...
            if fecha_limite_cobertura_cal and fecha_cobertura_cal:
                tiempo_disponible = (
                    fecha_limite_cobertura_cal - fecha_cobertura_cal
                ).days
                tiempo_disponible = (
                    tiempo_disponible if tiempo_disponible >= 0 else None
                )
            else:
            ...
            estatus = (
                "EN PROCESO"
                if not data.get("fecha_cobertura")
                else "CERRADA" if "fecha_ingreso" in data else "ESPERA DE INGRESO"
            )
            ...
            if imagen_dni_reverso:
                ...
                ruta_imagen_reverso = (
                    f"reclutamiento/static/img/controlplazas/{nombre_imagen_reverso}"
                )
            ...
            control_plaza = ControlDePlazas.objects.create(
                ...
                fecha_inicio_busqueda=(
                    parser.parse(data.get("fecha_inicio_busqueda"))
                    if data.get("fecha_inicio_busqueda")
                    else None
                ),
            ...
            return JsonResponse(
                {
                    "success": True,
                    "message": "Plaza registrada exitosamente",
                    "controlPlaza": control_plaza.id,
                },
                status=201,
            )
    ...
    if request.method == "DELETE":
        try:
            if not request.user.has_perm("Reclutamiento.delete_controldeplazas"):
                return JsonResponse(
                    {"error": "No tienes permisos para eliminar plazas"}, status=403
                )
            ...
            return JsonResponse(
                {"success": True, "message": "Plaza eliminada exitosamente"}, status=200
            )

        except ControlDePlazas.DoesNotExist:
            return JsonResponse(
                {"success": False, "message": "Control de plazas no encontrado"},
                status=404,
            )
        except Exception as e:
            return JsonResponse({"success": False, "message": str(e)}, status=500)
    ...
    if request.method == "PATCH":
        try:
            ...
            return JsonResponse(
                {"success": True, "message": "Estatus actualizado exitosamente"},
                status=200,
            )
        ...
        try:
            ...
            if imagen_dni:
                ...
                if control_plaza.nombreimagen:
                    old_image_path = os.path.join(
                        "reclutamiento/static/img/controlplazas",
                        control_plaza.nombreimagen,
                    )
            ...
            if imagen_dni_reverso:
                ...
                if control_plaza.nombreimagen1:  # Asegúrate de usar el campo correcto
                    old_image_path_reverso = os.path.join(
                        "reclutamiento/static/img/controlplazas",
                        control_plaza.nombreimagen1,
                    )
            ...
            control_plaza.fecha_inicio_busqueda = (
                parser.parse(data.get("fecha_inicio_busqueda"))
                if data.get("fecha_inicio_busqueda")
                else None
            )
            ...
            return JsonResponse(
                {"success": True, "message": "Plaza actualizada exitosamente"},
                status=200,
            )
...
def descargar_imagen(request, imagen_id):
    ...
    if not imagenes:
        return HttpResponse(
            status=404, content="No hay imágenes disponibles para descargar."
        )
...
class ExportExcelPlazas(View):
    def get(self, request, *args, **kwargs):
        template_path = os.path.join(
            "reclutamiento/static/templates/formatoplazaoriginal.xlsx"
        )

        # Cargar la plantilla de Excel
        try:
            wb = load_workbook(template_path)
            sheet = wb["CUADRO GENERAL DE PLAZAS"]
        except Exception as e:
            return HttpResponse(
                f"Error al cargar la plantilla de Excel: {e}", status=500
            )
        ...
        today = date.today()  # Fecha actual para cálculo de edad

        for idx, plaza in enumerate(control_de_plazas, start=start_row):
            try:
                sheet.cell(row=idx, column=2, value=plaza.usuario_creo.first_name)
                sheet.cell(
                    row=idx,
                    column=3,
                    value=plaza.sucursal.nombre_sucursal if plaza.sucursal else "",
                )
                sheet.cell(
                    row=idx,
                    column=4,
                    value=plaza.empresa.nombre_empresa if plaza.empresa else "",
                )
                sheet.cell(
                    row=idx,
                    column=5,
                    value=(
                        plaza.unidad_de_negocio.nombre_unidad_de_negocio
                        if plaza.unidad_de_negocio
                        else ""
                    ),
                )
                ...
                sheet.cell(
                    row=idx,
                    column=9,
                    value=plaza.modo.nombre_modo if plaza.modo else "",
                )
                sheet.cell(
                    row=idx,
                    column=10,
                    value=(
                        plaza.motivo_ingreso.nombre_motivo
                        if plaza.motivo_ingreso
                        else ""
                    ),
                )
                ...
                sheet.cell(
                    row=idx,
                    column=12,
                    value=plaza.puestos.nombre_puestos if plaza.puestos else "",
                )
                sheet.cell(
                    row=idx,
                    column=13,
                    value=(
                        plaza.departamento.nombre_departamento
                        if plaza.departamento
                        else ""
                    ),
                )
                sheet.cell(
                    row=idx,
                    column=14,
                    value=plaza.prioridad.nombre_prioridad if plaza.prioridad else "",
                )
                ...
                sheet.cell(
                    row=idx, column=22, value=plaza.tiempo_efectivo_fecha_ingreso
                )
                ...
                # Calcular edad y escribir en columna 32
                if plaza.fecha_nacimiento:
                    edad = (
                        today.year
                        - plaza.fecha_nacimiento.year
                        - (
                            (today.month, today.day)
                            < (plaza.fecha_nacimiento.month, plaza.fecha_nacimiento.day)
                        )
                    )
                    sheet.cell(row=idx, column=32, value=edad)
                else:
                    sheet.cell(row=idx, column=32, value="")

                sheet.cell(
                    row=idx,
                    column=33,
                    value=(
                        plaza.medio_reclutamiento.nombre_medio_de_reclutamiento
                        if plaza.medio_reclutamiento
                        else ""
                    ),
                )
                ...
                sheet.cell(
                    ...
                    column=40,
                    value=(
                        plaza.tipo_contrato.nombre_tipo_de_contrato
                        if plaza.tipo_contrato
                        else ""
                    ),
                )
            except Exception as e:
                return HttpResponse(
                    f"Error escribiendo datos en Excel en la fila {idx}: {e}",
                    status=500,
                )
        ...
        response["Content-Disposition"] = (
            'attachment; filename="Control_de_Plazas.xlsx"'
        )
...
def bolsaempleo_view(request, id=None):
    if request.method == "GET":
        ...
        medios_reclutamiento = MedioReclutamiento.objects.filter(activo=True).order_by(
        ...
        ciudad_id = request.GET.getlist("ciudad", [])

        # Convertir strings a enteros
        puestos_aspira = list(map(int, puestos_aspira)) if puestos_aspira else []
        puestos_aplica = list(map(int, puestos_aplica)) if puestos_aplica else []
        ciudad_id = list(map(int, ciudad_id)) if ciudad_id else []

        bolsa_empleos = (
            BolsaEmpleos.objects.all()
        ...
        if search:
            bolsa_empleos = bolsa_empleos.filter(
                ...
                | Q(
                    medio_reclutamiento__nombre_medio_de_reclutamiento__icontains=search
                )
        ...
        if puestos_aspira:
            bolsa_empleos = bolsa_empleos.filter(
                id__in=PuestoAspira.objects.filter(
                    puesto_id__in=puestos_aspira
                ).values_list("candidato_id", flat=True)
            )

        if puestos_aplica:
            bolsa_empleos = bolsa_empleos.filter(
                id__in=PuestoAplica.objects.filter(
                    puesto_id__in=puestos_aplica
                ).values_list("candidato_id", flat=True)
            )

        # Obtener puestos únicos para filtros, solo los usados
        puestos_aspira_filtro_ids = PuestoAspira.objects.values_list(
            "puesto_id", flat=True
        ).distinct()
        puestos_aplica_filtro_ids = PuestoAplica.objects.values_list(
            "puesto_id", flat=True
        ).distinct()

        puestos_aspira_filtro = Puestos.objects.filter(id__in=puestos_aspira_filtro_ids)
        puestos_aplica_filtro = Puestos.objects.filter(id__in=puestos_aplica_filtro_ids)
        ...
        # Agregar nombres de puestos para la tabla
        for empleo in page_obj:
            puestos_aspira_ids = list(
                PuestoAspira.objects.filter(candidato=empleo).values_list(
                    "puesto_id", flat=True
                )
            )
            puestos_aplica_ids = list(
                PuestoAplica.objects.filter(candidato=empleo).values_list(
                    "puesto_id", flat=True
                )
            )

            empleo.puestosaspira_ids = puestos_aspira_ids
            empleo.puestosaplica_ids = puestos_aplica_ids

            # Para mostrar nombres de puestos en tabla
            empleo.puestosaspira_nombres = list(
                Puestos.objects.filter(id__in=puestos_aspira_ids).values_list(
                    "nombre_puestos", flat=True
                )
            )
            empleo.puestosaplica_nombres = list(
                Puestos.objects.filter(id__in=puestos_aplica_ids).values_list(
                    "nombre_puestos", flat=True
                )
            )
        ...
        context = {
            "puestos": puestos,
            "puestos_en_bolsa": puestos_aspira_filtro,  # para filtro aspira
            "puestos_aplica_filtro": puestos_aplica_filtro,  # para filtro aplica
            "ciudades": ciudades,
            ...
            "estado": estado,
            "puestos_aspira": list(
                map(str, puestos_aspira)
            ),  # convertir a str para comparación en template
            "puestos_aplica": list(map(str, puestos_aplica)),
            "ciudad_id": list(map(str, ciudad_id)),
            "estados": estados,
        }
        ...
        try:
            if not request.user.has_perm("Reclutamiento.add_bolsaempleos"):
                return JsonResponse(
                    {"error": "No tienes permisos para agregar candidatos"}, status=403
                )
            ...
            nhijos = data.get("nhijos") or None
            ...
            if (
                not dni
                or not nombre_candidato
                or not telefono
                or not puestosaspira
                or not estado
            ):
                return JsonResponse(
                    {
                        "success": False,
                        "message": "Los campos DNI, Nombre del Candidato, Teléfono, Puesto al que Aspira y Estado son obligatorios.",
                    },
                    status=400,
                )
            ...
            if telefono == telefono2:
                return JsonResponse(
                    {
                        "success": False,
                        "message": "El teléfono principal y el secundario no pueden ser iguales.",
                    },
                    status=400,
                )

            if BolsaEmpleos.objects.filter(
                Q(telefono=telefono) | Q(telefono2=telefono)
            ).exists():
                return JsonResponse(
                    {
                        "success": False,
                        "message": "El número de teléfono principal ya está registrado.",
                    },
                    status=400,
                )
            ...
            if (
                telefono2
                and BolsaEmpleos.objects.filter(
                    Q(telefono=telefono2) | Q(telefono2=telefono2)
                ).exists()
            ):
                return JsonResponse(
                    {
                        "success": False,
                        "message": "El número de teléfono secundario ya está registrado.",
                    },
                    status=400,
                )
            ...
            if fecha_nacimiento:
                try:
                    fecha_nacimiento = timezone.datetime.strptime(
            ...
            if archivo:
                if archivo.content_type != "application/pdf":
                    return JsonResponse(
                        {
                            "success": False,
                            "message": "Solo se permiten documentos en formato PDF.",
                        },
                        status=400,
                    )
                ...
                ruta_relativa = os.path.join(
                    "reclutamiento", "static", "document", nuevo_nombre_archivo
                )
            ...
            puestos_validos = set(
                Puestos.objects.filter(id__in=puestosaspira).values_list(
                    "id", flat=True
                )
            )
            for puesto_id in puestosaspira:
                if int(puesto_id) in puestos_validos:
                    PuestoAspira.objects.create(
                        candidato=bolsa_empleo, puesto_id=puesto_id
                    )

            return JsonResponse(
                {"success": True, "message": "Candidato registrado exitosamente"},
                status=201,
            )

        except json.JSONDecodeError:
            return JsonResponse(
                {"success": False, "message": "Formato de JSON no válido."}, status=400
            )
        except Exception as e:
            return JsonResponse({"success": False, "message": str(e)}, status=500)
    ...
    if request.method == "POST" and id is not None:
        try:
            if not request.user.has_perm("Reclutamiento.change_bolsaempleos"):
                return JsonResponse(
                    {"error": "No tienes permisos para editar candidatos"}, status=403
                )

            data_json = request.POST.get("data")
            if not data_json:
                return JsonResponse(
                    {"success": False, "message": "No se recibieron datos."}, status=400
                )
            ...
            try:
                ...
                return JsonResponse(
                    {"success": False, "message": "El candidato no existe."}, status=404
                )
            ...
            archivo = request.FILES.get("cveditar")
            if archivo:
                if archivo.content_type != "application/pdf":
                    return JsonResponse(
                        {
                ...
                if bolsa_empleo.ruta and os.path.exists(
                    os.path.join(settings.BASE_DIR, bolsa_empleo.ruta)
                ):
            ...
            bolsa_empleo.telefono2 = data.get("telefono2") or None
            bolsa_empleo.ciudad_id = data.get("ciudad") or None
            bolsa_empleo.medio_reclutamiento_id = (
                data.get("mediosReclutamiento") or None
            )
            bolsa_empleo.fecha_nacimiento = fecha_nacimiento or None
            bolsa_empleo.estadocivil = data.get("estadocivil") or None
            bolsa_empleo.nhijos = data.get("nhijos") or None
            bolsa_empleo.direccion = data.get("direccion") or None
            bolsa_empleo.experiencia = data.get("experiencia") or None
            bolsa_empleo.observacion = data.get("observacion") or None
            bolsa_empleo.mediomovilizacion = data.get("mediomovilizacion") or None
            bolsa_empleo.usuario_modifico = request.user
            bolsa_empleo.save()

            # Actualizar puestos aspira
            PuestoAspira.objects.filter(candidato=bolsa_empleo).delete()
            for puesto_id in puestosaspira:
                PuestoAspira.objects.create(candidato=bolsa_empleo, puesto_id=puesto_id)

            # Actualizar puestos aplica
            PuestoAplica.objects.filter(candidato=bolsa_empleo).delete()
            for puesto_id in puestosaplica:
                PuestoAplica.objects.create(candidato=bolsa_empleo, puesto_id=puesto_id)

            return JsonResponse(
                {"success": True, "message": "Candidato actualizado exitosamente"},
                status=200,
            )
...
def descargar_pdf(request, archivo_id):
    ...
    if not archivo_path.exists():
        return HttpResponse(
            status=404, content="El archivo no se encuentra en el sistema de archivos."
        )

    # Leer el archivo y prepararlo para la descarga
    with open(archivo_path, "rb") as archivo:
        response = HttpResponse(
            archivo.read(), content_type=mimetypes.guess_type(str(archivo_path))[0]
        )
...
def exportar_bolsa_empleos(request):
    ...
    datos = BolsaEmpleos.objects.prefetch_related(
        "ciudad", "medio_reclutamiento"
    ).values(
        "id",
        "dni",
        "nombre_candidato",
    ...
    if "puestosaspira" in df.columns:
        df["puestosaspira"] = df["puestosaspira"].apply(
            lambda x: (
                ", ".join(
                    Puestos.objects.get(id=puesto_id).nombre_puestos for puesto_id in x
                )
    ...
    if "puestosaplica" in df.columns:
        df["puestosaplica"] = df["puestosaplica"].apply(
            lambda x: (
                ", ".join(
                    Puestos.objects.get(id=puesto_id).nombre_puestos for puesto_id in x
                )
...
def contratacionesform_view(request):
    if request.method == "GET":
        ...
        all_sucursales = (
            Sucursal.objects.filter(activo=True).order_by("nombre_sucursal").distinct()
        )
        all_empresas = (
            Empresas.objects.filter(activo=True).order_by("nombre_empresa").distinct()
        )
        all_unidades_negocio = (
            Unidad_Negocio.objects.filter(activo=True)
            .order_by("nombre_unidad_de_negocio")
        ...
        all_puestos = (
            Puestos.objects.filter(activo=True).order_by("nombre_puestos").distinct()
        )
        ...
        all_municipios = (
            MunicipioPais.objects.filter(activo=True)
            .order_by("nombre_municipio")
            .distinct()
        )
        all_departamentos_hn = (
            DepartamentoPais.objects.filter(activo=True)
            .order_by("nombre_departamento")
            .distinct()
        )
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_contratacionempleados"):
                return JsonResponse(
                    {"error": "No tienes permisos para agregar contrataciones"},
                    status=403,
                )
            ...
            if imagen:
                ...
                try:
                    os.makedirs(
                        os.path.dirname(os.path.join(settings.BASE_DIR, ruta_imagen)),
                        exist_ok=True,
                    )

                    # Guardar la imagen en la ruta especificada
                    with open(
                        os.path.join(settings.BASE_DIR, ruta_imagen), "wb+"
                    ) as destino:
...
def get_departamentos_por_pais(request, pais_id):
    departamentos = DepartamentoPais.objects.filter(
        pais_id=pais_id, activo=True
    ).values("id", "nombre_departamento")
    return JsonResponse(list(departamentos), safe=False)


def get_municipios_por_departamento(request, departamento_id):
    municipios = MunicipioPais.objects.filter(
        departamento_id=departamento_id, activo=True
    ).values("id", "nombre_municipio")
...
def contratacionestable_view(request, id=None):
    if request.method == "GET":
        ...
        municipios_options = set(
            contrataciones.values_list("municipio__nombre_municipio", flat=True)
        )
        puestos_options = set(
            contrataciones.values_list("puestos__nombre_puestos", flat=True)
        )
        unidades_negocio_options = set(
            contrataciones.values_list(
                "unidad_de_negocio__nombre_unidad_de_negocio", flat=True
            )
        )
        sucursales_options = set(
            contrataciones.values_list("sucursal__nombre_sucursal", flat=True)
        )
        fechas_ingreso_options = set(
            contrataciones.values_list("fecha_ingreso", flat=True)
        )
        ...
        if municipio_filter:
            contrataciones = contrataciones.filter(
                municipio__nombre_municipio__in=municipio_filter
            )

        if puesto_filter:
            contrataciones = contrataciones.filter(
                puestos__nombre_puestos__in=puesto_filter
            )

        if unidad_negocio_filter:
            contrataciones = contrataciones.filter(
                unidad_de_negocio__nombre_unidad_de_negocio__in=unidad_negocio_filter
            )

        if sucursal_filter:
            contrataciones = contrataciones.filter(
                sucursal__nombre_sucursal__in=sucursal_filter
            )

        if fecha_ingreso_filter:
            contrataciones = contrataciones.filter(
                fecha_ingreso__in=fecha_ingreso_filter
            )
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.change_contratacionempleados"):
                return JsonResponse(
                    {"error": "No tienes permisos para editar contrataciones"},
                    status=403,
                )
            ...
            unidad_negocio_id = (
                int(unidad_negocio)
                if unidad_negocio and unidad_negocio.isdigit()
                else None
            )
            ...
            tipo_contrato_id = (
                int(tipo_contrato)
                if tipo_contrato and tipo_contrato.isdigit()
                else None
            )
            ...
            departamento_empresa_id = (
                int(departamento_empresa)
                if departamento_empresa and departamento_empresa.isdigit()
                else None
            )
            ...
            nombre_empresa_id = (
                int(nombre_empresa)
                if nombre_empresa and nombre_empresa.isdigit()
                else None
            )
            ...
            if imagen:
                ...
                if (
                    contratacion.ruta
                ):  # Verifica si hay una imagen ya existente en la ruta
                    ...
                    if os.path.exists(ruta_anterior):
                        try:
                            ...
                            return JsonResponse(
                                {
                                    "success": False,
                                    "message": f"Error al eliminar la imagen anterior: {str(e)}",
                                },
                                status=500,
                            )
                ...
                try:
                    ...
                    return JsonResponse(
                        {
                            "success": False,
                            "message": f"Error al guardar la nueva imagen: {str(e)}",
                        },
                        status=500,
                    )
            ...
            contratacion.telefonoemergencia1 = (
                request.POST.get("telefonoEmergencia1") or None
            )
            ...
            contratacion.telefonoemergencia2 = (
                request.POST.get("telefonoEmergencia2") or None
            )
            ...
            contratacion.ultimogradodetalle = (
                request.POST.get("ultimogradodetalle") or None
            )
            contratacion.padecimiento = padecimiento
            contratacion.detalle_enfermedad = (
                request.POST.get("detalleEnfermedad") or None
            )
...
class ExportExcelContratacion(View):
    def get(self, request, *args, **kwargs):
        template_path = os.path.join(
            "reclutamiento/static/templates/FormatoContratacion.xlsx"
        )
        ...
        for idx, contratacion in enumerate(contrataciones, start=start_row):
            try:
                ...
                sheet.cell(
                    row=idx, column=start_col + 1, value=contratacion.tipo_contratacion
                )
                ...
                sheet.cell(
                    row=idx, column=start_col + 6, value=contratacion.fecha_nacimiento
                )
                ...
                sheet.cell(
                    ...
                    column=start_col + 7,
                    value=(
                        contratacion.departamento.nombre_departamento
                        if contratacion.departamento
                        else ""
                    ),
                )
                ...
                sheet.cell(
                    ...
                    column=start_col + 8,
                    value=(
                        contratacion.municipio.nombre_municipio
                        if contratacion.municipio
                        else ""
                    ),
                )
                ...
                sheet.cell(
                    row=idx, column=start_col + 12, value=contratacion.estado_civil
                )
                sheet.cell(
                    row=idx, column=start_col + 13, value=contratacion.profecion_oficio
                )
                ...
                sheet.cell(
                    row=idx, column=start_col + 15, value=contratacion.parentesco1
                )
                sheet.cell(
                    row=idx,
                    column=start_col + 16,
                ...
                sheet.cell(
                    row=idx, column=start_col + 18, value=contratacion.parentesco2
                )
                sheet.cell(
                    row=idx,
                    column=start_col + 19,
                ...
                sheet.cell(
                    row=idx, column=start_col + 20, value=contratacion.direccionexacta
                )
                sheet.cell(
                    row=idx, column=start_col + 21, value=contratacion.nivel_educativo
                )
                sheet.cell(
                    row=idx,
                    column=start_col + 22,
                ...
                sheet.cell(
                    ...
                    column=start_col + 25,
                    value=(
                        contratacion.puestos.nombre_puestos
                        if contratacion.puestos
                        else ""
                    ),
                )
                ...
                sheet.cell(
                    ...
                    column=start_col + 27,
                    value=(
                        contratacion.sucursal.nombre_sucursal
                        if contratacion.sucursal
                        else ""
                    ),
                )
                sheet.cell(
                    row=idx, column=start_col + 28, value=contratacion.fecha_ingreso
                )
                ...
                sheet.cell(
                    row=idx, column=start_col + 30, value=contratacion.dni_beneficiario
                )
                sheet.cell(
                    row=idx,
                    column=start_col + 31,
                ...
                sheet.cell(
                    row=idx, column=start_col + 32, value=contratacion.porcentaje
                )
                ...
                sheet.cell(
                    ...
                    column=start_col + 33,
                    value=(
                        contratacion.departamento_empresa.nombre_departamento
                        if contratacion.departamento_empresa
                        else ""
                    ),
                )
                ...
                sheet.cell(
                    ...
                    column=start_col + 34,
                    value=(
                        contratacion.unidad_de_negocio.nombre_unidad_de_negocio
                        if contratacion.unidad_de_negocio
                        else ""
                    ),
                )
                ...
                sheet.cell(
                    ...
                    column=start_col + 36,
                    value=(
                        contratacion.tipo_contrato.nombre_tipo_de_contrato
                        if contratacion.tipo_contrato
                        else ""
                    ),
                )
                ...
                sheet.cell(
                    row=idx, column=start_col + 37, value=contratacion.telefono_empresa
                )
                sheet.cell(
                    row=idx, column=start_col + 38, value=contratacion.direccionempresa
                )
                sheet.cell(
                    row=idx,
                    column=start_col + 39,
                    value=(
                        contratacion.nombre_empresa.nombre_empresa
                        if contratacion.nombre_empresa
                        else ""
                    ),
                )
                ...
                sheet.cell(
                    row=idx,
                    column=start_col + 40,
                    value="Si" if contratacion.hijos else "No",
                )
                ...
                return HttpResponse(
                    f"Error escribiendo datos en Excel en la fila {idx + 1}: {e}",
                    status=500,
                )
        ...
        response["Content-Disposition"] = (
            'attachment; filename="FormatoContratacion.xlsx"'
        )
    ...
    if request.method == "POST":
        try:
            if not request.user.has_perm("Reclutamiento.add_cesantias"):
                return JsonResponse(
                    {"error": "No tienes permisos para agregar cesantías"}, status=403
                )
            ...
            cesantia = Cesantias(
                ...
                autoriza=Jefes.objects.get(id=data["autoriza"]),
                empleado=Colaboradores.objects.get(id=data["empleado"]),
            ...
            # Responder con éxito
            return JsonResponse(
                {"success": True, "message": "Cesantía registrada correctamente."}
            )
        ...
        try:
            if not request.user.has_perm("Reclutamiento.change_cesantias"):
                return JsonResponse(
                    {"error": "No tienes permisos para editar cesantías"}, status=403
                )
            data = json.loads(request.body)
            ...
            cesantia.autoriza_id = data["autoriza"]
            cesantia.empleado_id = data["empleado"]
            ...
            return JsonResponse(
                {"success": True, "message": "Cesantía actualizada correctamente."}
            )
        except Exception as e:
            return JsonResponse({"success": False, "message": str(e)})
        ...
        cesantia_id = request.GET.get('cesantia_id')

        if cesantia_id:
            try:
                cesantia = Cesantias.objects.get(id=cesantia_id)

                return JsonResponse({
                    "cesantias":{
                            "id": cesantia.id,
                            "correlativo": cesantia.correlativo,
                            "autoriza": cesantia.autoriza_id,
                            "empleado": cesantia.empleado_id,
                            "fecha_inicial": cesantia.fecha_inicial,
                            "fecha_final": cesantia.fecha_final,
                            "año": cesantia.año,
                            "sueldo_actual": cesantia.sueldo_actual,
                            "porcentaje": cesantia.porcentaje,
                            "fecha_extencion": cesantia.fecha_extencion,
                            "estado_pago": cesantia.estado_pago,
                            "n_cheke": cesantia.n_cheke,                            
                    }
                })
            except Cesantias.DoesNotExist:
                return JsonResponse({"error": "Cesantia no encontrada"},status=404)
        else:
            ...
            all_autorizadoresselect = Jefes.objects.filter(
                activo=True
            ).filter(
                Q(puesto__nombre_puestos__iexact="GERENTE GENERAL") |
                Q(puesto__nombre_puestos__iexact="GERENTE PROPIETARIO")
            )

            all_empleados = Colaboradores.objects.filter(activo=True).order_by("nombrecolaborador").distinct()

            # Autorizadores únicos que están siendo usados en la tabla Cesantias
            all_autorizadores = Jefes.objects.filter(id__in=Cesantias.objects.values_list("autoriza__id", flat=True).distinct())

            # Obtener filtros desde GET
            search = request.GET.get("search", "")
            estados_pago = request.GET.getlist("estado_pago", [])
            años = request.GET.getlist("año", [])
            fechas_extencion = request.GET.getlist("fecha_extencion", [])
            autorizadores = request.GET.getlist("nombre_autoriza", [])  # 🔍 nuevo filtro aplicado
            ...
            if search:
                try:
                    ...
                    cesantias = cesantias.filter(
                        Q(empleado__nombrecolaborador__icontains=search)
            ...
            # ✅ Aplicar filtro de autorizadores
            if autorizadores:
                cesantias = cesantias.filter(autoriza__id__in=autorizadores)
            ...
            context = {
                ...
                "all_años": all_años,
                "all_fechas_extencion": all_fechas_extencion,
                "all_autorizadoresselect": all_autorizadoresselect,
                "all_autorizadores": all_autorizadores,  # ✅ Nuevo contexto
                "all_empleados": all_empleados,
                "estados_pago": estados_pago,
                "años": años,
                "fechas_extencion": fechas_extencion,
                "autorizadores": autorizadores,  # ✅ Filtro activo
            }
...
def imprimircesantias_view(request, id):
    ...
    # Extraer información del autorizador (jefe)
    jefe_autoriza = cesantia.autoriza
    dni_autorizador = jefe_autoriza.identidadjefe if jefe_autoriza else ""

    # Extraer información del colaborador
    colaborador = cesantia.empleado
    dni_colaborador = colaborador.dni if colaborador else ""
    empresa_colaborador = colaborador.empresa.nombre_empresa if colaborador and colaborador.empresa else ""
    sucursal_colaborador = colaborador.sucursal.nombre_sucursal if colaborador and colaborador.sucursal else ""
    departamento_colaborador = colaborador.departamento.nombre_departamento if colaborador and colaborador.departamento else ""
    ...
    context = {
        ...
        # Nuevos campos para mostrar en el template
        "dni_autorizador": dni_autorizador,
        "dni_colaborador": dni_colaborador,
        "empresa_colaborador": empresa_colaborador,
        "sucursal_colaborador": sucursal_colaborador,
...
def exportar_cesantias(request):
    datos = Cesantias.objects.select_related(
...
def perfilpuesto_view(request):
    ...
    all_tipos_perfil = PerfilesPuestos.objects.values_list(
        "tipo_perfil", flat=True
    ).distinct()
    all_empresas = PerfilesPuestos.objects.values_list("empresa", flat=True).distinct()
    all_departamentos = PerfilesPuestos.objects.values_list(
        "departamento", flat=True
    ).distinct()
    all_nombres_cargo = PerfilesPuestos.objects.values_list(
        "nombre_cargo", flat=True
    ).distinct()
    all_fechas_actualizacion = (
        PerfilesPuestos.objects.annotate(
            fecha_actualizacion_dia=TruncDate("modificado")
        )
    ...
    if fechas_actualizacion:
        try:
            fechas_seleccionadas = [
                datetime.strptime(f, "%d/%m/%Y") for f in fechas_actualizacion
            ]
            perfiles_puesto = perfiles_puesto.filter(
                modificado__date__in=fechas_seleccionadas
            )
        except ValueError:
...
def obtener_pruebas_por_nivel(request):
    if request.method == "GET":
        ...
        if nivel:
            ...
            return JsonResponse(
                {"success": False, "message": "Nivel no proporcionado"}, status=400
            )
...
def perfilpuestoregister_view(request):
    ...
    if request.method == "GET":
        ...
        if not request.user.has_perm("Reclutamiento.add_perfilespuestos"):
            return JsonResponse(
                {"error": "No tienes permisos para agregar perfiles de puesto"},
                status=403,
            )
        ...
        try:
            ...
            # 🧠 Validar que se haya enviado el nivel
            nivel_id = data.get("nivel_prueba")
            if not nivel_id:
                return JsonResponse(
                    {"success": False, "message": "Debe seleccionar el nivel de la prueba."},
                    status=400,
                )
            ...
            perfil = PerfilesPuestos.objects.create(
                ...
                empresa_id=data.get("empresa_select"),
                nombre_cargo_id=data.get("nombre_cargo"),
                departamento_id=data.get("departamento_select"),
                cargo_al_que_reporta_id=data.get("cargo_reportan_select"),
                educacion_universitario=data.get("educacion_input"),
                postgrado_especializaciones=data.get("postgrado_especialidades"),
                formacion_complementaria=data.get("formacion_complementaria"),
                idiomas=data.get("idioma_input"),
                nivel_idioma=data.get("nivel_idioma_select"),
                anos_experiencia=data.get("anosexperiencia_input"),
                pensamiento_estrategico=data.get("nivel_pensamiento_select"),
                enfoque_al_cliente=data.get("nivel_cliente_select"),
                planificacion_y_organizacion=data.get("nivel_planificacion_select"),
                comunicacion=data.get("nivel_comunicacion_select"),
                orientacion_al_logro=data.get("nivel_logro_select"),

                mision_cargo=data.get("mision_cargo"),
                ruta_organigrama=ruta_imagen,
                nombre_organigrama=nombre_imagen,
                residir_en_area=(data.get("residir_en_area") == "true"),
                hora_entrada=parse_time(data.get("hora_entrada")) if data.get("hora_entrada") else None,
                hora_salida=parse_time(data.get("hora_salida")) if data.get("hora_salida") else None,
                otros_considere=data.get("otros"),
                nivel_psicosmart_id=nivel_id,
                usuario_creo=request.user,

                montacargas=(data.get("montacargas") == "true"),
                equipo_pesado=(data.get("equipo_pesado") == "true")
            )


            # 🧷 Cargos que le reportan
            for id_puesto in json.loads(data.get("cargo_lereportan_select", "[]")):
                CargoReporta.objects.create(perfil=perfil, puesto_id=id_puesto)

            # ✅ Funciones del cargo
            funciones = json.loads(data.get("funcion_lista", "[]"))
            periodicidades = json.loads(data.get("periodicidad_lista", "[]"))
            importancias = json.loads(data.get("importancia_lista", "[]"))

            for i in range(len(funciones)):
                FuncionCargo.objects.create(
                    perfil=perfil,
                    funcion=funciones[i],
            ...
            # ❗ Retos/Dificultades
            for reto in json.loads(data.get("retos", "[]")):
                PerfilDificultades.objects.create(perfil=perfil, descripcion=reto)

            # 🧰 Materiales y equipos
            for material in json.loads(data.get("materiales_equipos", "[]")):
                MaterialEquipo.objects.create(perfil=perfil, descripcion=material)

            # 💸 Plan de compensación
            for plan in json.loads(data.get("plan_de_compensacion", "[]")):
                PlanCompensacionPerfil.objects.create(perfil=perfil, descripcion=plan)

            # 🎁 Beneficios Promaco
            for beneficio in json.loads(data.get("beneficiospromaco", "[]")):
                BeneficioPromaco.objects.create(perfil=perfil, descripcion=beneficio)

            # 📈 Indicadores
            for indicador in json.loads(data.get("principales_indicadores", "[]")):
                IndicadorPerfil.objects.create(perfil=perfil, descripcion=indicador)

            # 🧠 Psicosmart pruebas automáticas según el nivel
            pruebas = Spicosmart.objects.filter(nivel=nivel_id, activo=True)
            for prueba in pruebas:
                SpicosmartAsignadas.objects.create(perfil=perfil, prueba=prueba)

            return JsonResponse({
                "success": True,
                "message": "Perfil de puesto creado correctamente",
                "id": perfil.id,
            })
            ...
            print(f"Error: {str(e)}")
            return JsonResponse(
                {"success": False, "message": f"Error al procesar la solicitud: {str(e)}"},
                status=500,
            )
...
def imprimir_perfilpuesto_view(request, perfil_id):
    ...
    try:
        ids = ast.literal_eval(perfil_puesto.cargo_al_que_reporta)
        ids = [int(i) for i in ids]
        puestos_relacionados = Puestos.objects.filter(id__in=ids)
    except (ValueError, SyntaxError, TypeError):
        puestos_relacionados = []  

    context = {
        "perfil_puesto": perfil_puesto,
        "puestos_relacionados": puestos_relacionados,

        "cargos_que_reportan": perfil_puesto.cargos_reporta.all(),
        "funciones": perfil_puesto.funciones.all(),
        "retos": perfil_puesto.retos.all(),
        "materiales_equipos": perfil_puesto.materiales_equipos.all(),
        "planes_de_compensacion": perfil_puesto.planes_de_compensacion.all(),
        "beneficios_promaco": perfil_puesto.beneficios_promaco.all(),
        "indicadores": perfil_puesto.indicadores.all(),
        "pruebas_psicosmart": perfil_puesto.pruebas_psicosmart.all(), 
    }
...
def update_perfilpuesto_view(request, id):
    if request.method == "GET":
        ...
        all_empresas = Empresas.objects.filter(activo=True).order_by("nombre_empresa")
        all_puestos = Puestos.objects.filter(activo=True).order_by("nombre_puestos")
        all_departamentos = Departamento.objects.filter(activo=True).order_by("nombre_departamento")

        ids_seleccionados = list(perfil_puesto.cargos_reporta.values_list("puesto_id", flat=True))

        funciones_cargo = FuncionCargo.objects.filter(perfil=perfil_puesto)
        dificultades = PerfilDificultades.objects.filter(perfil=perfil_puesto)
        indicadores = IndicadorPerfil.objects.filter(perfil=perfil_puesto)

        materiales_guardados = list(perfil_puesto.materiales_equipos.values_list("descripcion", flat=True))
        materiales_predefinidos = [
            "COMPUTADOR", "CELULAR", "UNIFORME", "ESCRITORIO",
            "LICENCIA SAP", "LICENCIA IVEND", "SILLA", "CORREO"
        ]
        materiales_seleccionados = [m for m in materiales_guardados if m in materiales_predefinidos]
        materiales_otros = [m for m in materiales_guardados if m not in materiales_predefinidos]

        # Compensación
        compensaciones_predefinidas = [
            "SALARIO BASE", "COMISION", "BONO",
            "DEPRECIACION Y COMBUSTIBLE", "HOSPEDAJE"
        ]
        compensaciones_guardadas = list(perfil_puesto.planes_de_compensacion.values_list("descripcion", flat=True))
        compensaciones_seleccionadas = [c for c in compensaciones_guardadas if c in compensaciones_predefinidas]
        compensaciones_otros = [c for c in compensaciones_guardadas if c not in compensaciones_predefinidas]

        # Beneficios Promaco
        beneficios_predefinidos = [
            "SEGURO MEDICO PRIVADO", "SEGURO SOCIAL", "PERMISO DE ESTUDIO",
            "BONO SUPERMERCADO", "BONO CAFE PROMACO",
            "DESCUENTO EN SUPERMERCADO", "DESCUENTO EN CLINICA Y LAVORATORIOS",
            "SEGURO DE VEHICULO"
        ]
        beneficios_guardados = list(perfil_puesto.beneficios_promaco.values_list("descripcion", flat=True))
        beneficios_seleccionados = [b for b in beneficios_guardados if b in beneficios_predefinidos]
        beneficios_otros = [b for b in beneficios_guardados if b not in beneficios_predefinidos]


        nivel_spicosmart = perfil_puesto.nivel_psicosmart
        ...
        context = {
            ...
            "dificultades_perfil": dificultades,

            "materiales_predefinidos": materiales_predefinidos,
            "materiales_seleccionados": materiales_seleccionados,
            "materiales_otros": materiales_otros,

            "compensaciones_predefinidas": compensaciones_predefinidas,
            "compensaciones_seleccionadas": compensaciones_seleccionadas,
            "compensaciones_otros": compensaciones_otros[0] if compensaciones_otros else "",

            "beneficios_predefinidos": beneficios_predefinidos,
            "beneficios_seleccionados": beneficios_seleccionados,
            "beneficios_otros": beneficios_otros[0] if beneficios_otros else "",

            "hora_entrada": perfil_puesto.hora_entrada.strftime('%H:%M') if perfil_puesto.hora_entrada else "",
            "hora_salida": perfil_puesto.hora_salida.strftime('%H:%M') if perfil_puesto.hora_salida else "",

            "nivel_spicosmart": nivel_spicosmart,
    ...
    if request.method == 'POST':
        ...
        try:
            perfil_id = request.POST.get('perfilPuestoId')
            perfil = get_object_or_404(PerfilesPuestos, id=perfil_id)

            tipo_perfil = request.POST.get('tipo_perfil')
            empresa_id = request.POST.get('empresa_select')
            nombre_cargo = request.POST.get('nombre_cargo')
            departamento_id = request.POST.get('departamento_select')
            cargo_reportan_id = request.POST.get('cargo_reportan_select')
            lereportan_json = request.POST.get('cargo_lereportan_select')
            lereportan_ids = json.loads(lereportan_json) if lereportan_json else []

                # Otros campos...
            educacion = request.POST.get('educacion_input')
            postgrado_especialidades = request.POST.get('postgrado_especialidades')
            formacion_complementaria = request.POST.get('formacion_complementaria')
            idioma = request.POST.get('idioma_input')
            nivel_idioma = request.POST.get('nivel_idioma_select')
            anosexperiencia = request.POST.get('anosexperiencia_input')


            nivel_pensamiento = request.POST.get('nivel_pensamiento_select')
            nivel_cliente = request.POST.get('nivel_cliente_select')
            nivel_planificacion = request.POST.get('nivel_planificacion_select')
            nivel_comunicacion = request.POST.get('nivel_comunicacion_select')
            nivel_logro = request.POST.get('nivel_logro_select')


            mision_cargo = request.POST.get('mision_cargo')

            perfil.tipo_perfil = tipo_perfil
            perfil.empresa_id = empresa_id
            perfil.nombre_cargo_id = nombre_cargo
            perfil.departamento_id = departamento_id
            perfil.cargo_reportan_id = cargo_reportan_id
            perfil.educacion_universitario = educacion
            perfil.postgrado_especializaciones = postgrado_especialidades
            perfil.formacion_complementaria = formacion_complementaria
            perfil.idiomas = idioma
            perfil.nivel_idioma = nivel_idioma
            perfil.anos_experiencia = anosexperiencia


            perfil.pensamiento_estrategico = nivel_pensamiento
            perfil.enfoque_al_cliente = nivel_cliente
            perfil.planificacion_y_organizacion = nivel_planificacion
            perfil.comunicacion = nivel_comunicacion
            perfil.orientacion_al_logro = nivel_logro
            perfil.mision_cargo = mision_cargo

                # Imagen
            imagen = request.FILES.get("ruta_organigrama")
            nombre_imagen = perfil.nombre_organigrama
            ruta_imagen = perfil.ruta_organigrama
            ...
            # Funciones
            funciones_json = request.POST.get('funcion_lista', '[]')
            periodicidades_json = request.POST.get('periodicidad_lista', '[]')
            importancias_json = request.POST.get('importancia_lista', '[]')

            funciones = json.loads(funciones_json)
            periodicidades = json.loads(periodicidades_json)
            importancias = json.loads(importancias_json)

            FuncionCargo.objects.filter(perfil_id=perfil_id).delete()
            for funcion, periodicidad, importancia in zip(funciones, periodicidades, importancias):
                if funcion.strip():
                    FuncionCargo.objects.create(perfil=perfil, funcion=funcion.strip(), periodicidad=periodicidad.strip(), importancia=importancia.strip())


            MaterialEquipo.objects.filter(perfil_id=perfil_id).delete()
            lista_materiales = request.POST.get('materiales_equipos', '[]')
            lista_materiales = json.loads(lista_materiales)
            for descripcion in lista_materiales:
                if descripcion.strip():
                    MaterialEquipo.objects.create(perfil_id=perfil_id, descripcion=descripcion.strip())

                # Dificultades
            PerfilDificultades.objects.filter(perfil_id=perfil_id).delete()
                # Crear las nuevas dificultades
            dificultades_json = request.POST.get('dificultades', '[]')
            lista_dificultades = json.loads(dificultades_json)
            for descripcion in lista_dificultades:
                if descripcion.strip():
                    PerfilDificultades.objects.create(perfil_id=perfil_id, descripcion=descripcion)


                # Residir, horarios y otros
            perfil.residir_en_area = request.POST.get('residir_en_area') == 'true'
            perfil.hora_entrada = request.POST.get('hora_entrada')
            perfil.hora_salida = request.POST.get('hora_salida')
            perfil.otros_considere = request.POST.get('otros')
            perfil.montacargas = request.POST.get('montacargas') == 'true'
            perfil.equipo_pesado = request.POST.get('equipo_pesado') == 'true'
            perfil.usuario_modifico = request.user

                # Planes de compensación
            compensaciones_json = request.POST.get('plan_de_compensacion', '[]')
            compensaciones = json.loads(compensaciones_json)
            perfil.planes_de_compensacion.all().delete()
            for descripcion in compensaciones:
                if descripcion.strip():
                    PlanCompensacionPerfil.objects.create(perfil=perfil, descripcion=descripcion)

                # Beneficios
            beneficios_json = request.POST.get('beneficiospromaco', '[]')
            beneficios = json.loads(beneficios_json)
            perfil.beneficios_promaco.all().delete()
            for descripcion in beneficios:
                if descripcion.strip():
                    BeneficioPromaco.objects.create(perfil=perfil, descripcion=descripcion)


                # Spicosmart asignadas
            nivel_id = request.POST.get('nivel_prueba')
            SpicosmartAsignadas.objects.filter(perfil=perfil).delete()
            pruebas = Spicosmart.objects.filter(nivel=nivel_id, activo=True)
            for prueba in pruebas:
                SpicosmartAsignadas.objects.create(perfil=perfil, prueba=prueba)

                # Indicadores
            indicadores_json = request.POST.get('principales_indicadores', '[]')
            indicadores = json.loads(indicadores_json)
            perfil.indicadores.all().delete()
            for desc in indicadores:
               if desc.strip():
                    IndicadorPerfil.objects.create(perfil=perfil, descripcion=desc)

                # Cargo que le reportan
            CargoReporta.objects.filter(perfil=perfil).delete()
            for puesto_id in lereportan_ids:
                CargoReporta.objects.create(perfil=perfil, puesto_id=puesto_id)


            perfil.nivel_psicosmart_id = nivel_id
            perfil.save()
...
def updatecompleto_perfilpuesto_view(request, id):
    if request.method == "GET":
        ...
        try:
            ids_seleccionados = ast.literal_eval(perfil_puesto.cargo_que_le_reportan)
            ids_seleccionados = [str(id).strip() for id in ids_seleccionados]
        except (ValueError, SyntaxError, TypeError):
            ids_seleccionados = []
        ...
        predefined_beneficios = [
            ...
            "DESCUENTO EN CLINICA Y LAVORATORIOS",
            "SEGURO DE VEHICULO",
        ]

        # Limpiar espacios extra en plan de compensación y filtrar "Otros"
        plan_de_compensacion_limpio = [
            comp.strip() for comp in perfil_puesto.plan_de_compensacion
        ]
        otros_compensacion = [
            comp
            for comp in plan_de_compensacion_limpio
            if comp not in predefined_compensacion
        ]
...
def contratacionesform_multi_view(request):
    if request.method == "GET":
        ...
        all_municipios = (
            MunicipioPais.objects.all().order_by("nombre_municipio").distinct()
        )
        all_departamentos_hn = (
            DepartamentoPais.objects.all().order_by("nombre_departamento").distinct()
        )
        ...
        return render(
            request, "contrataciones_multi/registrarcontrataciones_multi.html", context
        )
    if request.method == "POST":
        try:
            if not request.user.has_perm(
                "Reclutamiento.add_contratacionempleadosmulti"
            ):
                return JsonResponse(
                    {"error": "No tienes permisos para registrar registros"}, status=403
                )
            tipo_contratacion = request.POST.get("tipoIngreso") or None
            primer_nombre = request.POST.get("primerNombre") or None
            segundo_nombre = request.POST.get("segundoNombre") or None
...
def contratacionesmulti_view(request, id=None):
    if request.method == "GET":
        ...
        # Obtener valores únicos para los filtros
        municipios_options = set(
            contrataciones_multi.values_list("municipio__nombre_municipio", flat=True)
        )
        puestos_options = set(contrataciones_multi.values_list("puestos", flat=True))
        sucursales_options = set(
            contrataciones_multi.values_list("sucursal__nombre_sucursal", flat=True)
        )
        fechas_ingreso_options = set(
            contrataciones_multi.values_list("fecha_ingreso", flat=True)
        )
        ...
        )  # Crear un rango de páginas a mostrar
        ...
        return render(
            request, "contrataciones_multi/contrataciones_multi.html", context
        )


def imprimir_contratacion_multi_view(request, contratacion_id):
    contratacion_multi = get_object_or_404(
        ContratacionEmpleadosmulti, id=contratacion_id
    )
    ...
    return render(
        request, "contrataciones_multi/imprimir_contratacion_multi.html", context
    )
...
def updatecontrataciones_multi_view(request, contratacion_id):
    ...
    if request.method == "GET":
        ...
        return render(
            request, "contrataciones_multi/updatecontrataciones_multi.html", context
        )

    if request.method == "POST":
        try:
            if not request.user.has_perm(
                "Reclutamiento.change_contratacionempleadosmulti"
            ):
                return JsonResponse(
                    {"error": "No tienes permisos para editar registros"}, status=403
                )
            ...
            parentesco_beneficiario = (
                request.POST.get("parentesco_beneficiario") or None
            )
            porcentaje_beneficiario = request.POST.get("porcentajeBeneficiario") or None
            ...
            if imagen:
                ...
                if contratacion_multi.ruta:
                    ruta_anterior = os.path.join(
                        settings.BASE_DIR, contratacion_multi.ruta
                    )
                    ...
                    if os.path.exists(ruta_anterior):
                        try:
                            ...
                            print(
                                f"Imagen anterior eliminada: {ruta_anterior}"
                            )  # Log para depuración
                        except Exception as e:
                        ...
                        print(
                            f"La ruta de la imagen anterior no existe: {ruta_anterior}"
                        )  # Log para depuración
...
class ExportExcelContratacionMulti(View):
    def get(self, request, *args, **kwargs):
        template_path = os.path.join(
            "reclutamiento/static/templates/FormatoContratacion_multi.xlsx"
        )
        ...
        for idx, contratacion in enumerate(contrataciones, start=start_row):
            try:
                ...
                sheet.cell(
                    row=idx,
                    column=start_col + 7,
                    value=contratacion.pais.nombre_pais if contratacion.pais else "",
                )
                sheet.cell(
                    row=idx,
                    column=start_col + 8,
                    value=(
                        getattr(contratacion.departamento, "nombre_departamento", "")
                        if contratacion.departamento
                        else ""
                    ),
                )
                ...
                sheet.cell(
                    row=idx, column=start_col + 14, value=contratacion.estado_civil
                )

                # Datos de contacto de emergencia
                sheet.cell(
                    row=idx, column=start_col + 15, value=contratacion.nombre_emergencia
                )
                sheet.cell(
                    row=idx, column=start_col + 16, value=contratacion.parentesco
                )
                sheet.cell(
                    row=idx,
                    column=start_col + 17,
                    value=contratacion.telefonoemergencia,
                )

                # Enfermedad
                sheet.cell(
                    row=idx, column=start_col + 18, value=contratacion.enfermedad
                )

                # Dirección exacta y último grado de estudio
                sheet.cell(
                    row=idx, column=start_col + 19, value=contratacion.direccionexacta
                )
                sheet.cell(
                    row=idx,
                    column=start_col + 20,
                ...
                sheet.cell(
                    row=idx, column=start_col + 24, value=contratacion.fecha_ingreso
                )
                ...
                sheet.cell(
                    row=idx, column=start_col + 26, value=contratacion.dni_beneficiario
                )
                sheet.cell(
                    row=idx,
                    column=start_col + 27,
                ...
                sheet.cell(
                    row=idx, column=start_col + 28, value=contratacion.porcentaje
                )
            except Exception as e:
                return HttpResponse(
                    f"Error escribiendo datos en Excel en la fila {idx + 1}: {e}",
        ...
        response["Content-Disposition"] = (
            'attachment; filename="FormatoContratacion_multi.xlsx"'
        )
...
def telefonosinventario_view(request, id=None):
    if request.method == "GET":
        if not request.user.has_perm("Reclutamiento.view_inventariotelefonos"):
            return redirect("sinacceso")
        all_nombres_telefono = Inventariotelefonos.objects.values_list(
            "nombretelefono", flat=True
        ).distinct()
        ...
        if search:
            inventarios = inventarios.filter(
                Q(nombretelefono__icontains=search) | Q(correlativo__icontains=search)
            )
        ...
        try:
            if not request.user.has_perm("Reclutamiento.add_inventariotelefonos"):
                return JsonResponse(
                    {"error": "No tienes permisos para crear registros"}, status=403
                )
            ...
            if not nombre_telefono or valor_telefono <= 0 or cantidad <= 0:
                return JsonResponse(
                    {"success": False, "message": "Datos no válidos."}, status=400
                )
            ...
            return JsonResponse(
                {
                    "success": True,
                    "message": f"{cantidad} teléfonos registrados exitosamente.",
                },
                status=201,
            )
        ...
        if id:
            ...
            telefono.delete()
            return JsonResponse(
                {"success": True, "message": "Teléfono eliminado con éxito."}
            )

        return JsonResponse(
            {"success": False, "message": "ID de teléfono no encontrado."}
        )
        ...
        try:
            ...
            if valortelefono is not None:
                telefono.valortotal = float(
                    valortelefono
                )  # Asegúrate de convertir a float
            ...
            if estado is not None:
                ...
                if estado == "REGISTRADO" and telefono.estado in [
                    "ASIGNADO",
                    "REASIGNADO",
                ]:
            ...
            return JsonResponse(
                {"success": True, "message": "Teléfono actualizado con éxito."}
            )

        except json.JSONDecodeError:
            return JsonResponse(
                {"success": False, "message": "Error al decodificar los datos JSON."}
            )
        except Exception as e:
            return JsonResponse({"success": False, "message": str(e)})
...
def obtener_id_telefono(request):
    ...
    if nombre_telefono and estadotelefono:
        ...
        if telefono:
            ...
            return JsonResponse(
                {
                    "success": False,
                    "message": "No se encontró el teléfono en el inventario.",
                },
                status=404,
            )
    ...
    return JsonResponse(
        {
            "success": False,
            "message": "Parámetros nombretelefono o estadotelefono no proporcionados.",
        },
        status=400,
    )
...
def telefonos_view(request):
    if request.method == "GET":
        ...
        all_telefonosinventario = (
            Inventariotelefonos.objects.filter(estado="REGISTRADO")
        ...
        try:
            ...
            inventario_telefono1 = Inventariotelefonos.objects.filter(
                id=telefono_id, estado="REGISTRADO"
            ).first()
            ...
            estado_telefono = (
                inventario_telefono1.estadotelefono
            )  # Obtiene el `estadotelefono` desde `Inventariotelefonos`

            # Validación de campos obligatorios
            if (
                not nombre
                or not dni
                or not unidad_negocio_id
                or not departamento_id
                or not telefono_id
                or not caracter
            ):
                return JsonResponse(
            ...
            return JsonResponse(
                {
                    "success": True,
                    "message": "Registro de telefonía y actualización de inventario exitosos.",
                },
                status=201,
            )
            ...
            return JsonResponse(
                {"success": False, "message": "Error en los datos enviados."},
                status=400,
            )
        except Exception as e:
            return JsonResponse({"success": False, "message": str(e)}, status=500)
...
def imprimir_telefono_view(request, id):
    ...
    inventario_telefono = get_object_or_404(
        Inventariotelefonos, id=telefonia.nombretelefono
    )

    telefonia.valor_formateado = (
        "{:,.2f}".format(telefonia.valor) if telefonia.valor else None
    )
    if telefonia.quinsena:
        # Verificar si el número es entero o tiene decimales
        if telefonia.quinsena.is_integer():
            telefonia.quincena_formateada = "{:,.0f}".format(
                telefonia.quinsena
            )  # Sin decimales
        else:
            telefonia.quincena_formateada = "{:,.2f}".format(
                telefonia.quinsena
            )  # Con dos decimales
    else:
...
def telefonosreasignar_view(request, correlativo):
    if request.method == "PUT":
        try:
            ...
            inventario_telefono = Inventariotelefonos.objects.filter(
                correlativo=correlativo, estado="ASIGNADO"
            ).first()
            ...
            return JsonResponse(
                {"success": True, "message": "Registro reasignado correctamente"}
            )

        except Telefonia.DoesNotExist:
            return JsonResponse(
                {"success": False, "message": "Correlativo no encontrado"}, status=404
            )
...
def deduccion_telefono(request):
    if request.method == "POST":
        ...
        telefono_inventario = get_object_or_404(
            Inventariotelefonos, correlativo=correlativo
        )
        ...
        meses_uso = (
            (fecha_extravio_dt.year - fecha_asignacion.year) * 12
            + (fecha_extravio_dt.month - fecha_asignacion.month)
            + 1
        )
...
def exportar_inventario_telefonos(request):
    ...
    datos = Inventariotelefonos.objects.values(
        "id",
        "nombretelefono",
        "correlativo",
        "estado",
