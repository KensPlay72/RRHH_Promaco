...
document.getElementById('update-form-puesto').addEventListener('submit', function(event) {
    ...
    const data = {
        id: document.getElementById('id_puesto_editar').value,
...
function llenarFormularioEditar(boton) {
    const idpuestos = boton.getAttribute('data-id');

    fetch(`?puestos_id=${idpuestos}`)
        .then(response => response.json())
        .then(data => {
            const puestos = data.puestos;
    
            document.getElementById('id_puesto_editar').value = puestos.id;
            document.getElementById('nombre_puesto_editar').value = puestos.nombre_puestos;
            document.getElementById('estadoeditar').value = puestos.activo ? "true" : "false";
